...
import com.bf.omex.gate.config.KafkaProperties;
...
class OmexGateKafkaResourceIT {
    ...
    void setup() {
        ...
        OmexGateKafkaResource kafkaResource = new OmexGateKafkaResource(kafkaProperties);
    ...
    void producesMessages() throws Exception {
        restMockMvc
            .perform(post("/api/omex-gate-kafka/publish/topic-produce?message=value-produce"))
    ...
    void consumesMessages() throws Exception {
        ...
        MvcResult mvcResult = restMockMvc
            .perform(get("/api/omex-gate-kafka/consume?topic=topic-consume"))
            .andExpect(status().isOk())
